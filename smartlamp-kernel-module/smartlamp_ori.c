...
static int usb_send_cmd(const char *cmd, int param) {
    ...
    ret = usb_bulk_msg(smartlamp_device, usb_sndbulkpipe(smartlamp_device, usb_out), usb_out_buffer, min(strlen(usb_out_buffer), MAX_RECV_LINE), &actual_size, 1000);
    if (ret) {
    ...
    while (retries > 0) {
        mdelay(100);
        // Lê dados da USB
        memset(usb_in_buffer, 0, 512);
        ret = usb_bulk_msg(smartlamp_device, usb_rcvbulkpipe(smartlamp_device, usb_in), usb_in_buffer, 512, &actual_size, 1000);
        if(ret == -110){
            printk(KERN_INFO "SmartLamp: read timeout");
            break;
        }
        else if (ret)
        {
        ...
        // printk(KERN_INFO "Tentativa[%d]: %s | %d\n", actual_size, usb_in_buffer, '\0', usb_max_size);
    ...
    //     printk(KERN_INFO "%c %u %x\n", BUFFER[k], BUFFER[k], BUFFER[k]);
    // }
    // printk("-------");
    // for(int k = 0; k < strlen(resp_expected); k++) {
    //     printk(KERN_INFO "--> %c %u %x\n", resp_expected[k], resp_expected[k], resp_expected[k]);
    // }
    char **cmd_value = extract_cmd_value(BUFFER);
    long value = -1;
    printk(KERN_INFO "SmartLamp: CMD: %s |  Val: %s\n", cmd_value[0], cmd_value[1]);
    // printk(KERN_INFO "%s | %s\n", BUFFER, resp_expected);

    // for(int k = 0; k < strlen(cmd_value[1]); k++) {
    //     printk(KERN_INFO "--> %c %u %x\n", cmd_value[1][k], cmd_value[1][k], cmd_value[1][k]);
    // }

    int aux = kstrtol(cmd_value[1], 10, &value);
    
    printk(KERN_INFO "SmartLamp: kstrtol = %d\n", aux);
    

    // if(strcmp(resp_expected, cmd_value[0]) == 0)
    // {
    //     return 0;
    // }
    kfree(cmd_value);
    kfree(BUFFER);
    return value; // Não recebi a resposta esperada do dispositivo
}
